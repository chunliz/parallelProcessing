...
__global__ void simulateKernal(double* E, double* E_prev, double* R,
    ...
    if((row<m)&&(col<n)){
        ...
        E[(row+1)*(n+2)+col+1] = E[(row+1)*(n+2)+col+1]
                -dt*(kk* E[(row+1)*(n+2)+col+1]*(E[(row+1)*(n+2)+col+1]
                - a)*(E[(row+1)*(n+2)+col+1]-1)+ E[(row+1)*(n+2)+col+1] *R[(row+1)*(n+2)+col+1]);

        R[(row+1)*(n+2)+col+1] = R[(row+1)*(n+2)+col+1] + dt*(epsilon+M1* R[(row+1)*(n+2)+col+1]
                /( E[(row+1)*(n+2)+col+1]+M2))*(-R[(row+1)*(n+2)+col+1]-kk* E[(row+1)*(n+2)+col+1]*(E[(row+1)*(n+2)+col+1]-b-1));
    }
