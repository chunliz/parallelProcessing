...
int main(int argc, char* argv[])
{
  ...
  for (i = 0; i < n; ++i) {
    ...
    for(i = 0 ; i < n ; i++){
        count=0;
#pragma offload target(mic) in(n) inout(count,my_count) inout(a,temp:length(n))
{
#pragma omp parallel num_threads(numberOfThreads) \
private(my_count) reduction(+:count)
{
	 my_count=0;
//#pragma offload target(mic) in(n) inout(my_count) in(a:length(n))
#pragma omp for
        for(j = 0 ; j < n ; j++){
            if(a[j] < a[i])
                my_count++;
...
#pragma offload target(mic) in(n) inout(a,temp:length(n))
{
